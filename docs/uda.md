...
- **The DB hostname is a DNS dependency, and a *quiet* app is the most exposed.** Every read and write funnels through one pool addressed by *container name*, so anything disturbing Docker's embedded resolver (127.0.0.11) takes the whole data surface down at once. Hit on 2026-07-27: a co-tenant ML container grew to 4.75GB RSS and OOM-starved the shared host, dockerd degraded under the pressure, and this app logged `UnknownHostException: ksh-postgres` from `ConnectionPool$PoolCleaner` six times (22:16–22:17, then again at 22:11 the next night when the same cron fired). The failing path is `testAllIdle` → `reconnectIfExpired`, which walks only **idle** connections — a busy app recovers on the next borrow, an idle one has all of them ageing out on `maxAge` (10 min) with no traffic to force recovery *or* raise an alarm. The sibling OBT app sat broken ~40h this way. Mitigated in the `Dockerfile`: `preferIPv4Stack` (musl fires A and AAAA in parallel; asking for A only sidesteps it — and note `--dns-option single-request-reopen` is a **glibc** option, a no-op on this alpine image), `networkaddress.cache.ttl=60` (the JVM otherwise re-resolves about once per reconnect, forever), `negative.ttl=0`, plus a `HEALTHCHECK` that also resolves the DB host — because `/` renders fine with no database and read healthy throughout the outage. If it recurs, check `journalctl | grep -E "invoked oom-killer|failed to query external DNS server"` on the host before suspecting the app.
- **`ENV JAVA_OPTS` beside `CMD ["java","-jar",…]` is dead config.** `java -jar` never reads `JAVA_OPTS` — only Spring Boot's script launcher does. Prod ran on the JVM's ergonomic default (25% of *host* RAM, ~3.9g on the deploy box) while the Dockerfile claimed `-Xmx512m`. Fixed with `ENTRYPOINT ["sh","-c","exec java $JAVA_OPTS -jar /app/app.jar"]` — `exec` matters, so the JVM stays PID 1 and still receives SIGTERM on `docker stop`.
