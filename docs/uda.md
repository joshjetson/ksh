...
- **A helper param named `<association>Id` collides with the derived FK on save.** GORM exposes a read-only `courseId` accessor for a `course` association. If a `save`/`update` to a domain with that association also carries a param literally named `courseId` (e.g. a `courseId` you pass for the next-view data instructions), the binder tries to set that read-only property and the whole save fails with `Cannot set readonly property: courseId`. Name such helper params something that isn't `<assoc>Id` (e.g. `forCourse`). `delete` is unaffected because it doesn't bind. Hit when wiring `CourseReward` attach.
